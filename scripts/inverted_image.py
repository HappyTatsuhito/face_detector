...
for image_path in os.listdir(path + '/face_image/'):
    ...
    for image_file in os.listdir(path + '/face_image/'+image_path):
        ...
        cv2.imwrite(path + '/face_image2/' + image_path + '/' + image_file.split('_')[0] + '_' + str(int(image_file.split('_')[1])+2) + '_' + image_file.split('_')[2], cv2.flip(img, 1))
        
        f = open(path + '/learning/' + image_path + '/' + image_file[:-3] + 'txt')
        learning_txt = f.read()
        f.close()
        print image_file
        a, converted_x, converted_y, converted_w, converted_h = learning_txt.split(' ')
        ...
        label_txt = open(path + '/learning2/' + image_path + '/' + image_file.split('_')[0] + '_' + str(int(image_file.split('_')[1])+2) + '_' + image_file.split('_')[2][:-3] + 'txt', 'w')
